...
import MaterialDatatableToolbar from "../src/MaterialDatatableToolbar";
import MaterialDatatableSearch from "../src/MaterialDatatableSearch";
import textLabels from "../src/textLabels";

describe("<MaterialDatatableToolbar />", function() {
  let data;
  let columns;
  let options;
  ...
  it("should render a toolbar", () => {
    const mountWrapper = mount(<MaterialDatatableToolbar columns={columns} data={data} options={options} />);
  ...
  it("should render a toolbar with no search icon if option.search = false", () => {
    ...
    const mountWrapper = mount(<MaterialDatatableToolbar columns={columns} data={data} options={newOptions} />);
  ...
  it("should render a toolbar with a search clicking search icon", () => {
    const shallowWrapper = shallow(<MaterialDatatableToolbar columns={columns} data={data} options={options} />)
      .dive()
      .dive()
      .dive();
    ...
    const actualResult = shallowWrapper.find(MaterialDatatableSearch);
  ...
  it("should hide search after clicking cancel icon", () => {
    ...
    const shallowWrapper = shallow(
      <MaterialDatatableToolbar searchTextUpdate={searchTextUpdate} columns={columns} data={data} options={options} />,
    )
      .dive()
      .dive()
      .dive();
    ...
    let actualResult = shallowWrapper.find(MaterialDatatableSearch);
    assert.strictEqual(actualResult.length, 1);
    ...
    actualResult = shallowWrapper.find(MaterialDatatableSearch);
    assert.strictEqual(actualResult.length, 0);
  });
